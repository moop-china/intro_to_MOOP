...
1.  **MOOP为用户提供了创作和分享的平台。**
    ...
    MOOP鼓励用户进行内容的创作并贡献出来，创建自己的专题，并学习别人的专题，在专题之下进行讨论。
...
3.  **MOOP提供可定制的内容。**
    ...
    得益于Jupyter Notebook的服务，MOOP可以整合notebook到一起，形成一个专题式的学习内容。用户只需要将内容整理好，然后按照MOOP的指引进行上传。所有的内容都是用户自己定制的，主题可以是leetcode那样的算法，也可以是优矿那样的金融量化分析，甚至可以没有程序，只有文字。
...
​    MOOP建议围绕一个主题来组织内容，称之为一个专题，比如“量化金融分析”，“Python爬虫实战”。创建一个新的专题，需要以下的信息。

1.  专题本身的信息

    名称，图片，专题的描述，考核内容，专题特点和参考资料
...
4.  专题的权限
    ...
    是否公开，如果不公开需要添加学生，一个私有的专题，那么赋予某一部分人访问的权限是很有必要的。通过直接上传一个csv或者是excel表格，MOOP会自动地查找到这些学生，然后赋予他们访问权限。

5.  数据

    对于数据相关的专题，实验的流程都是相同的，但是数据的不同就有不同的分析结果，所以MOOP专门提供了上传自己的数据的地方。

​    在每个专题被发布后，如果有任何不满意的地方，都可以再次编辑，不用担心一次做不好的情况。在专题页面，老师还可以对学生的学习进度进行观察和评估。

​    对于学生而言，和老师看到的内容是不一样的。除了上面提到的实验内容，专题描述，先修知识，考核内容，专题特点和参考，学生在专题页面再没有多余的信息。也就是说，学生就只能进行学习，而老师不仅可以管理专题，还可以管理学生。



#### 实验

​    每个专题下都有数个实验，每次实验有几个部分(也就是几个notebook)，如果内容不算多，那么一个实验就足够了，就好像一本书的章节一样。 想象一个编程的新手，要学习Python，在“Python入门”专题中，可能会学习下面的知识：
...
​    MOOP自带了很多实验模板，同时老师也可以自己创建实验。比较遗憾的是，目前MOOP无法做到自主创建实验，而是要老师联系管理员，将实验，也就是notebooks递交给内容建设员，让内容建设员来创建实验，创建好后，老师再在专题页面拉取想要的实验。
...
​    下面用现实场景来解释一下这个体系：一个学校为一个单位，即租户，这个租户中有少量的管理员，和大量的老师学生。管理员可以直接跟MOOP平台联系，获得技术支持，并管理这个租户内的全部账户，但是并不能创建新的内容。一般而言，具有一定的专业知识的管理员都可以申请成为CONTRIBUTOR。而学校里的教师，可以从管理员处获得老师账户，可以创建新的专题，当老师写了一些新的notebook，可以联系CONTRIBUTOR来创建新的实验，然后用到自己的专题之中。这些实验一般在一个租户下是公用的，也就是说，同校的教师和学生是共享这些实验内容的，大家都可以互相使用对方的实验来组成自己的专题。而不同校的实验内容，可以看到，但是就不一定能访问。学生就只能学习由老师创建的专题，其他没有任何权限。
...
2. 开始编程
   ...
   在专题浏览页面，可以看到一个专题下面的一些实验，点击启动按钮再等待一段时间便可以启动任意的一个实验。由于实验与实验之间的环境是隔离开的，所以不同的实验需要重新启动新的环境。每次启动大约需要10s，相当于是电脑重新开机的时间。

   启动后，会跳转到一个新的界面，就是Jupyter Notebook的界面，在这里，你的使用就和以前的使用完全一样了。如果你并没有使用过，可以看一下[第二节](#2.1-Jupyter-Notebook介绍)的对Jupyter的介绍。

3. 管理自己的信息

   在右上角的头像处，可以查看已修专题，也可以修改自己的个人信息。
...
​    一个内容平台的核心竞争力是内容本身。MOOP本身能提供的内容资源非常有限，任何一个平台都是如此，一个良好的社区是靠大家来共同维护和建造的，而不是靠社区管理员。作为一个新的内容平台，无可避免地要从无到有。就像一个黑洞，越吞越大才能更有吸引力。当实验材料足够多的时候，就可以随心所欲地组织材料，进行修改，创建你想要的专题了。

​    一个开放的平台，还是需要亮点来吸引人们聚集。而MOOP平台的亮点就在于，提供了统一的云平台进行编程和内容的分享，并有相当程度的自定义空间。即，这是一个欢迎内容创作者的平台，MOOP提供了很大的空间来让创作者创建自己的专题。

​    MOOP拥抱共享开放的思想，同时尊重版权，如果有不想公开的内容，MOOP提供了专题和实验的私有选项，只供你选择的对象使用。
